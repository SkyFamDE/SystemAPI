...
public class MySQL_Language {
    ...
    public MySQL_Language(String HOST, Integer PORT, String USERNAME, String PASSWORD) {
        this.host = HOST;
        this.port = PORT;
    ...
    public void connect() {
        ...
        if(!isConnected()) {
            try {
                connection = DriverManager.getConnection("jdbc:mysql://" + this.host + ":" + this.port + "/language", this.username, this.password);
                Bukkit.getConsoleSender().sendMessage(SystemAPI.getInstance().strings.System + "§aLanguage");
